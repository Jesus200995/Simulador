...
export function createPremiumMarker(
  ...
  const borderCol = selected ? '#ffffff' : c.border;
  
  // Mismo diseño exacto que AlertasAdminPage, pero con stroke para "contorno"
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="${c.start}" stroke="${borderCol}" stroke-width="${strokeWidth}" width="${size}" height="${size}" style="filter: drop-shadow(0 ${selected ? '4px 8px' : '2px 4px'} rgba(0,0,0,0.35)); transition: all 0.2s ease;">
