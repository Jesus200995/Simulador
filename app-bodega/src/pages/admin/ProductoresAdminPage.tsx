    ...
    <div className="space-y-6">
      ...
      <div className="flex border-b border-white/5 gap-2">
        ...
        <button 
          ...
          className={`px-4 py-3 text-[9.5px] font-bold border-b-2 transition-all ${
            tab === 'todos' 
              ? 'border-emerald-500 text-gray-900' 
              : 'border-transparent text-gray-500 hover:text-gray-700'
          }`}
      ...
      <div className="bg-white/80 border border-white/5 rounded-2xl p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
        ...
        <div className="relative">
          ...
          <input 
            ...
            className="w-full bg-gray-50 border border-white/5 rounded-xl pl-9 pr-4 py-2.5 text-[9.5px] text-gray-900 placeholder-gray-500 outline-none focus:border-emerald-500/50"
          />
        </div>
        ...
        <select
          ...
          className="w-full bg-gray-50 border border-white/5 rounded-xl px-3.5 py-2.5 text-[9.5px] text-gray-900 outline-none focus:border-emerald-500/50"
        >
        ...
        {tab === 'todos' ? (
          ...
          <div className="flex items-center justify-end px-2">
            <span className="text-[9.5px] text-gray-500 font-semibold uppercase tracking-wider">
      ...
      <div className="flex-shrink-0">
        <button
          ...
          className="flex items-center gap-2 text-[10px] font-bold text-gray-500 hover:text-gray-900 transition-all px-1 mb-2"
        >
          ...
          return (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-2">
              <div className="bg-white/80 border border-white/5 rounded-2xl p-4 space-y-1">
                ...
                <p className="text-[9.5px] font-black text-gray-900 leading-none">{hombres}</p>
                <p className="text-[9.5px] text-gray-500">productores</p>
              </div>
              <div className="bg-white/80 border border-white/5 rounded-2xl p-4 space-y-1">
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Mujeres</p>
                <p className="text-[9.5px] font-black text-gray-900 leading-none">{mujeres}</p>
                <p className="text-[9.5px] text-gray-500">productoras</p>
              </div>
              <div className="bg-white/80 border border-white/5 rounded-2xl p-4 space-y-1">
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Edad Promedio</p>
                <p className="text-[9.5px] font-black text-gray-900 leading-none">{edadPromedio}<span className="text-[10px] text-gray-500 ml-1">años</span></p>
                <p className="text-[9.5px] text-gray-500">de {edades.length} con CURP</p>
              </div>
              <div className="bg-white/80 border border-white/5 rounded-2xl p-4 space-y-1">
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Mayores de 60</p>
                <p className="text-[9.5px] font-black text-gray-900 leading-none">{mayores60}</p>
                <p className="text-[9.5px] text-gray-500">adultos mayores</p>
              </div>
            </div>
          );
        })()}
      ...
      <div className="bg-white/80 border border-white/5 rounded-2xl overflow-hidden shadow-sm">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <p className="text-[9.5px] text-gray-500">Cargando padrón de productores...</p>
          </div>
        ) : paginatedList.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-center px-4">
            <ShieldAlert size={36} className="text-gray-500" />
            <p className="text-[10px] text-gray-500 font-bold">Sin productores encontrados</p>
            <p className="text-[10px] text-gray-500">No hay registros que coincidan con los criterios de búsqueda.</p>
          </div>
        ) : (
                ...
                {paginatedList.map(prod => (
                  <tr key={prod.id} className="hover:bg-white/[0.01] transition-all">
                    <td className="py-4 px-5">
                      <div>
                        <p className="font-extrabold text-gray-900 text-[10px]">{prod.nombre} {prod.apellidos}</p>
                        <p className="text-[9.5px] text-gray-500 mt-0.5">{prod.email || 'Sin correo'}</p>
                      </div>
                    </td>
                    <td className="py-4 px-5 font-mono text-[10px] text-gray-500">{prod.curp}</td>
                    <td className="py-4 px-5">
                      <p className="font-bold text-gray-800">{prod.up_municipio}</p>
                      <p className="text-[9.5px] text-gray-500">{prod.up_estado}</p>
                    </td>
                    ...
                    <td className="py-4 px-5 text-right space-x-1.5">
                      ...
                      {prod.estado_validacion === 'activo' && (
                        <button 
                          ...
                          className="p-1.5 px-2 bg-white/5 hover:bg-red-500/10 text-gray-500 hover:text-red-600 rounded-lg text-[9.5px] font-bold transition-all border border-white/5"
                        >
                          Suspender
                        </button>
                      )}
                      ...
                      {prod.estado_validacion === 'suspendido' && (
                        <button 
                          ...
                          className="p-1.5 px-2 bg-white/5 hover:bg-emerald-500/10 text-gray-500 hover:text-emerald-600 rounded-lg text-[9.5px] font-bold transition-all border border-white/5"
                        >
                          Reactivar
                        </button>
                      )}
