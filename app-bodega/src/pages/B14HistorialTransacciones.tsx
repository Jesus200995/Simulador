    ...
    <div className="w-full">
      ...
      <div className="w-full bg-gradient-to-br from-[#1A5C38] via-[#1e6b42] to-[#22733f] rounded-b-3xl shadow-[0_8px_30px_rgba(26,92,56,0.25)] relative overflow-hidden group/banner">
        ...
        <div className="w-full mx-auto px-4 sm:px-5 lg:px-10 xl:px-16 pt-4 pb-5 relative z-10 transition-transform duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover/banner:translate-x-1">
          ...
          <h1 className="text-[9.5px] sm:text-[9.5px] font-bold text-white leading-tight drop-shadow-sm">Transacciones</h1>
          ...
          {txs.length > 0 && (
            <button
              ...
              className="mt-3 inline-flex items-center gap-2 text-[9.5px] font-bold text-white bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl px-4 py-2 transition-all duration-300 hover:shadow-lg active:scale-[0.98]"
            >
      ...
      <div className="w-full mx-auto px-4 sm:px-5 lg:px-10 xl:px-16 py-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {txs.map(tx => (
            <div key={tx.id}
              ...
              className="bg-white rounded-[1.5rem] border border-black/[0.04] shadow-[0_2px_8px_rgba(0,0,0,0.02)] p-4 cursor-pointer hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] hover:-translate-y-0.5 hover:border-black/[0.08] transition-all duration-500 group/card">
              <div className="flex justify-between items-start gap-3 mb-2">
                <p className="font-bold text-[10px] text-gray-900 group-hover/card:text-[#1A5C38] transition-colors truncate flex-1">
                  {tx.nombre_productor || tx.nombre_productor_libre || 'Productor'}
                </p>
                <span className={`text-[9.5px] font-bold px-2 py-0.5 rounded-full flex-shrink-0 ${estadoBadge[tx.confirmacion_productor] || 'bg-gray-100 text-gray-500'}`}>
                  {tx.confirmacion_productor}
                </span>
              </div>
              <p className="text-[9.5px] text-gray-500 font-medium">{tx.bodega_nombre} · {tx.fecha}</p>
              <p className="text-[9.5px] font-black text-[#1A5C38] mt-2">
                {tx.volumen_ton} ton · ${formatNum(tx.precio_ton)}/ton
              </p>
              <p className="text-[10px] text-gray-400 mt-1 font-medium">{tx.tipo_maiz}</p>
            </div>
          ))}
        </div>
        ...
        {!loading && txs.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <p className="font-semibold text-[10px] text-gray-700">Sin transacciones registradas</p>
            <p className="text-[10px] text-gray-400">Registra la primera compra de maíz</p>
          </div>
        )}
