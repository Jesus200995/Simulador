      ...
      <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16 gap-3 text-center">
            ...
            <button onClick={() => navigate('/oferta')}
              className="mt-3 bg-[#1e5b4f] text-white px-5 py-3 rounded-[1.25rem] text-[14px] font-bold shadow-[0_4px_12px_rgba(26,92,56,0.2)] hover:shadow-[0_8px_24px_rgba(26,92,56,0.3)] hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98] transition-all duration-300">
              ...
              <div key={d.id} className="bg-white rounded-2xl border border-black/[0.04] shadow-[0_2px_10px_rgba(0,0,0,0.04)] p-4 flex flex-col gap-3 hover:shadow-[0_8px_24px_rgba(0,0,0,0.08)] hover:-translate-y-0.5 hover:border-black/[0.08] transition-all duration-300 group/card">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 transition-transform duration-500 group-hover/card:translate-x-1">
                    <p className="font-bold text-[17px] text-gray-900 group-hover/card:text-[#1e5b4f] transition-colors truncate">{d.municipio}</p>
                    <p className="text-[13px] text-gray-400 font-medium flex items-center gap-1 mt-0.5">
                      <MapPin size={11} /> {d.estado || 'Sin estado'}
                    </p>
                  </div>
                  {d.tipo_maiz && (
                    <span className="bg-[#1e5b4f]/[0.08] text-[#1e5b4f] text-[11px] font-semibold px-2.5 py-1 rounded-full flex-shrink-0">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className={`rounded-xl p-3 ${Number(d.productores_disponibles) >= 10 ? 'bg-emerald-50' : Number(d.productores_disponibles) >= 5 ? 'bg-amber-50' : 'bg-[#e8f5f3]'}`}>
                    <div className="flex items-center gap-1 mb-1">
                      <Users size={11} className="text-gray-400" />
                      <p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wide">Productores</p>
                    </div>
                    <p className="text-[20px] font-black text-[#1e5b4f]">{d.productores_disponibles}</p>
                  </div>
                  <div className={`rounded-xl p-3 ${Number(d.toneladas_estimadas) >= 500 ? 'bg-emerald-50' : Number(d.toneladas_estimadas) >= 100 ? 'bg-amber-50' : 'bg-[#e8f5f3]'}`}>
                    <div className="flex items-center gap-1 mb-1">
                ...
                <div className="flex gap-2 mt-auto">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 bg-[#1e5b4f]/[0.08] hover:bg-[#1e5b4f]/[0.13] text-[#1e5b4f] rounded-xl py-2.5 text-[12px] font-bold active:scale-[0.98] transition-all duration-200"
                  >
