  ...
  return (
    <div className="w-full">
      ...
      <div className="w-full bg-gradient-to-br from-[#1A5C38] via-[#1e6b42] to-[#22733f] rounded-b-3xl shadow-[0_8px_30px_rgba(26,92,56,0.25)] relative overflow-hidden group/banner">
        ...
        <div className="w-full mx-auto px-4 sm:px-5 lg:px-10 xl:px-16 pt-4 pb-5 flex items-center gap-4 relative z-10 transition-transform duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover/banner:translate-x-1">
          <div className="w-14 h-14 rounded-[1.25rem] bg-white/20 backdrop-blur-sm shadow-inner flex items-center justify-center flex-shrink-0 transition-transform duration-500 group-hover/banner:scale-110 group-hover/banner:-rotate-6">
            <span className="text-[9.5px] font-bold text-white drop-shadow-sm">{initials}</span>
          </div>
          <div>
            <p className="text-[9.5px] font-bold text-white leading-tight drop-shadow-sm">{user?.nombre_completo || 'Usuario'}</p>
            <p className="text-green-100/80 text-[10px] mt-0.5 font-medium">{user?.email || ''}</p>
            <span className="inline-block mt-1.5 bg-white/20 backdrop-blur-sm border border-white/10 text-white text-[9.5px] font-bold px-2.5 py-0.5 rounded-full capitalize">{user?.rol || 'Bodega'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="w-full mx-auto px-4 sm:px-5 lg:px-10 xl:px-16 py-5 space-y-6">
        ...
        <div>
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 transition-colors hover:text-[#1A5C38]/60">Herramientas</p>
            ...
            {ACCIONES.map(({ icon: Icon, label, desc, path, iconBg, iconColor }) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0 transition-transform duration-500 group-hover/card:translate-x-1">
                  <p className="text-[10px] font-bold text-gray-900 group-hover/card:text-[#1A5C38] transition-colors truncate">{label}</p>
                ...
                <ChevronRight size={13} className="text-gray-300 flex-shrink-0 transition-transform duration-300 group-hover/card:translate-x-1 group-hover/card:text-[#1A5C38]" />
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-[1.5rem] border border-black/[0.04] shadow-[0_2px_8px_rgba(0,0,0,0.02)] px-5 py-5 flex items-center gap-4 transition-all duration-500 hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] hover:-translate-y-0.5 hover:border-black/[0.08] group/card">
          ...
          <div className="transition-transform duration-500 group-hover/card:translate-x-1">
            <p className="text-[10px] font-black text-[#1A5C38] tracking-tight">SIMAC</p>
            <p className="text-[9.5px] text-gray-500 font-medium mt-0.5">Sistema de Ordenamiento · Maíz Blanco México</p>
          </div>
        </div>
      </div>
    </div>
