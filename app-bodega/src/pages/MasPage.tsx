...
const ACCIONES = [
  { icon: UserCircle2, label: 'Mi perfil',                    desc: 'Ver y editar tu cuenta',          path: '/perfil',         iconBg: 'bg-[#1e5b4f]/[0.08]', iconColor: 'text-[#1e5b4f]', primary: true },
        ...
        {ACCIONES.map(({ icon: Icon, label, desc, path, iconBg, iconColor, primary }) => (
          <button
            ...
            className={`w-full flex items-center gap-4 px-5 py-5 rounded-[1.5rem] border active:scale-[0.98] transition-all duration-300 text-left group/card
              ${primary
                ? 'bg-[#1e5b4f] border-[#1e5b4f] shadow-[0_4px_18px_rgba(26,92,56,0.25)] hover:shadow-[0_8px_28px_rgba(26,92,56,0.35)] hover:-translate-y-0.5'
          ...
          >
            ...
            <div className="flex-1 min-w-0 transition-transform duration-300 group-hover/card:translate-x-0.5">
              <p className={`text-[15px] font-bold leading-tight ${primary ? 'text-white' : 'text-gray-900 group-hover/card:text-[#1e5b4f] transition-colors'}`}>{label}</p>
            ...
            <ChevronRight size={18} className={`flex-shrink-0 transition-transform duration-300 group-hover/card:translate-x-1 ${primary ? 'text-white/60' : 'text-gray-300 group-hover/card:text-[#1e5b4f]'}`} />
          </button>
        ))}
      </div>
