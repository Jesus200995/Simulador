...
export default function CicloProductivoPage() {
  ...
  const inputCls = 'w-full bg-slate-50/50 border border-slate-200 rounded-[14px] px-3.5 py-3 text-[10px] font-semibold text-slate-800 placeholder-slate-400 focus:border-[#1A5C38] focus:bg-white focus:ring-2 focus:ring-[#1A5C38]/10 transition-all outline-none';
  ...
  if (cargandoCiclos) {
    return (
      <div className="flex flex-col items-center justify-center w-full min-h-screen bg-[#f4f5f7] gap-3">
        ...
        <p className="text-[9.5px] font-semibold text-slate-400">Cargando tus ciclos…</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col font-sans w-full min-h-screen bg-[#f4f5f7] pb-[40px]">
        <div className="w-full bg-gradient-to-b from-[#1A5C38] to-[#124227] rounded-b-[28px] shadow-[0_8px_30px_rgba(26,92,56,0.15)] relative z-10 overflow-hidden">
          <div className="max-w-[700px] mx-auto px-4 sm:px-5 pt-5 pb-7 relative z-20">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-[9.5px] font-bold text-emerald-100/90 uppercase tracking-widest bg-black/10 px-3 py-1 rounded-full backdrop-blur-sm">
                Mis ciclos
              </div>
              <div className="w-9" />
            </div>
            <h1 className="text-[10px] font-black text-white tracking-tight">Ciclos productivos</h1>
            <p className="text-[9.5px] text-emerald-100/70 font-medium mt-0.5">Tu siembra registrada en SIMAC</p>
          </div>
        </div>

        <div className="w-full max-w-[700px] mx-auto px-4 sm:px-5 -mt-3 relative z-20">
          <div className="max-w-[500px] mx-auto">
            <h3 className="text-[10px] font-bold text-slate-700 mb-3 mt-2">
              Ciclos registrados
            </h3>
              ...
              {ciclosExistentes.map(ciclo => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`px-3 py-1 rounded-full text-[9.5px] font-bold shrink-0 ${
                      estado === 'activo'
  ...
  return (
    <div className="flex flex-col font-sans w-full min-h-screen bg-[#f4f5f7] pb-[100px]">
      ...
      {/* Contenido principal */}
      <div className="w-full max-w-[700px] mx-auto px-4 sm:px-5 -mt-4 relative z-20">
        <div className="max-w-[500px] mx-auto">

          {error && (
            <div className="mb-4 p-3.5 bg-red-50/90 backdrop-blur-sm border border-red-100 rounded-[16px] text-red-700 text-[9.5px] font-medium flex items-start gap-3 shadow-sm animate-in fade-in slide-in-from-top-4">
              <AlertTriangle size={13} className="shrink-0 text-red-600 mt-0.5" />
              <span className="leading-relaxed">{error}</span>
            </div>
          )}

          {/* Tarjeta Contenedor */}
          <div className="bg-white/95 backdrop-blur-xl rounded-[24px] p-4 sm:p-7 shadow-[0_8px_25px_rgb(0,0,0,0.03)] border border-white transition-all duration-300">
              ...
              <div className="space-y-6 relative z-10 animate-in fade-in slide-in-from-right-4 duration-300">
                {esPrimerLogin && (
                  <div className="bg-indigo-50/50 border border-indigo-100 rounded-[14px] p-3.5 mb-2 flex gap-3 items-start">
                    ...
                    <div>
                      <p className="text-indigo-900 text-[9.5px] font-bold tracking-tight">Un último paso</p>
                      <p className="text-indigo-700/80 text-[10px] font-medium mt-0.5 leading-relaxed">
                ...
                <div className="text-center sm:text-left mb-2">
                  ...
                  <h2 className="text-[10px] sm:text-[9.5px] font-black text-slate-900 tracking-tight leading-tight mb-1.5">
                    Tu ciclo productivo
                  </h2>
                  <p className="text-[9.5px] text-slate-500 font-medium">¿Qué ciclo estás sembrando actualmente?</p>
                </div>
                  ...
                  {CICLOS.map(c => (
                    <button key={c.valor} onClick={() => setForm(f => ({...f, cycle_type: c.valor}))}
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`text-[10px] font-black tracking-tight ${form.cycle_type === c.valor ? 'text-[#1A5C38]' : 'text-slate-800'}`}>
                          {c.label}
                        </p>
                        <p className={`text-[10px] font-medium mt-0.5 ${form.cycle_type === c.valor ? 'text-emerald-700/80' : 'text-slate-500'}`}>
                          {c.desc}
                        </p>
                      </div>
                ...
                <div className="pt-4 border-t border-slate-100">
                  <p className="text-[10px] font-bold text-slate-800 mb-2.5">¿En qué año?</p>
                  <div className="flex gap-2.5">
                    {[AÑO_ACTUAL - 1, AÑO_ACTUAL, AÑO_ACTUAL + 1].map(y => (
                      <button key={y}
                        onClick={() => setForm(f => ({...f, cycle_year: y}))}
                        className={`flex-1 py-2.5 rounded-[12px] border-2 font-black text-[10px] transition-all active:scale-95
                          ${form.cycle_year === y 
            ...
            {paso === 2 && (
              <div className="space-y-6 relative z-10 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="text-center sm:text-left mb-2">
                  ...
                  <p className="text-[9.5px] text-slate-500 font-medium">Selecciona el tipo de maíz que produces.</p>
                </div>
                ...
                {!tipoMaiz ? (
                      ...
                      <button key={t} onClick={() => setTipoMaiz(t as any)}
                        ...
                        <div className="flex items-center gap-3.5">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center text-slate-500 group-hover:text-[#1A5C38] group-hover:bg-[#1A5C38]/10 bg-slate-50 transition-colors`}>
                            {idx === 0 ? <Wheat size={12} /> : idx === 1 ? <Sun size={12} /> : <Sprout size={12} />}
                          </div>
                          <p className="text-[9.5px] font-black text-slate-800 capitalize group-hover:text-[#1A5C38] transition-colors tracking-tight">
                            Maíz {t}
                          </p>
                        </div>
                        <ChevronLeft size={13} className="text-slate-300 rotate-180 group-hover:text-[#1A5C38] group-hover:translate-x-1 transition-all" strokeWidth={3} />
                      </button>
                    ))}
                  </div>
                ) : (
                  <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                    <div className="flex items-center justify-between mb-4 bg-slate-50/80 p-2.5 rounded-[14px] border border-slate-100 shadow-sm">
                      <p className="font-black text-slate-800 capitalize text-[10px] flex items-center gap-2 px-2">
                        {tipoMaiz === 'blanco' ? <Wheat size={13} className="text-[#1A5C38]"/> : tipoMaiz === 'amarillo' ? <Sun size={13} className="text-amber-500"/> : <Sprout size={13} className="text-emerald-500"/>} 
                        Maíz {tipoMaiz}
                      </p>
                      <button onClick={() => { setTipoMaiz(''); setForm(f => ({...f, variety_id: '', variety_other: ''})); setEsCriollo(false); }} 
                        className="text-[#1A5C38] text-[10px] font-bold bg-white border border-slate-200 px-3 py-1.5 rounded-full hover:bg-slate-50 transition-colors shadow-sm active:scale-95">
                        Cambiar
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-2 mb-4 max-h-[240px] overflow-y-auto pr-1.5 scrollbar-thin scrollbar-thumb-slate-200">
                      {variedades.map(v => (
                        <button key={v.code}
                          ...
                          <span className={`text-[9.5px] font-bold ${form.variety_id === v.code ? 'text-[#1A5C38]' : 'text-slate-700'}`}>
                            {v.label}
                          </span>
                        </button>
                      ))}
                    ...
                    {esCriollo && (
                      <div className="animate-in fade-in slide-in-from-top-2 p-4 bg-slate-50/80 rounded-[16px] border border-slate-100 shadow-sm">
                        <label className="block text-[9.5px] font-bold text-slate-800 mb-2">
            ...
            {paso === 3 && (
              <div className="space-y-6 relative z-10 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="text-center sm:text-left mb-2">
                  ...
                  <p className="text-[9.5px] text-slate-500 font-medium">Estima las dimensiones de tu siembra actual.</p>
                </div>
                ...
                <div className="space-y-3.5">
                  <div className="p-4 bg-slate-50/80 border border-slate-100 rounded-[16px] shadow-sm">
                    <label className="block text-[10px] font-bold text-slate-800 mb-2.5 text-center sm:text-left">
                    ...
                    <div className="flex items-center gap-2.5">
                      <input type="number" min="0.1" step="0.1" max={areaHaCalc ?? 9999}
                        ...
                        className={`${inputCls} text-[10px] font-black text-center flex-1 py-3 shadow-inner`}
                      />
                      <span className="text-slate-400 font-bold text-[10px] px-2">ha</span>
                    </div>
                    {areaHaCalc && !form.area_sown_ha && (
                      <button onClick={() => setForm(f => ({...f, area_sown_ha: String(areaHaCalc)}))}
                        className="mt-3 w-full text-blue-700 text-[9.5px] font-bold border border-blue-200 bg-white py-2.5 rounded-full hover:bg-blue-50 hover:border-blue-300 transition-all shadow-sm active:scale-95">
                  ...
                  <div className="p-4 bg-slate-50/80 border border-slate-100 rounded-[16px] shadow-sm">
                    <label className="block text-[10px] font-bold text-slate-800 mb-1 text-center sm:text-left">
                      Rendimiento esperado
                    </label>
                    <p className="text-[10px] text-slate-500 mb-3 font-medium text-center sm:text-left">Por hectárea (ton/ha)</p>
                    <div className="flex items-center gap-2.5">
                      <input type="number" min="0.1" max="30" step="0.1"
                        ...
                        className={`${inputCls} text-[10px] font-black text-center flex-1 py-3 shadow-inner`}
                      />
                      <span className="text-slate-400 font-bold text-[10px] px-2 leading-tight">ton<br/><span className="text-[9.5px]">/ ha</span></span>
                    </div>
                  </div>
                </div>

                {form.area_sown_ha && form.yield_expected && (
                  <div className="bg-gradient-to-br from-[#1A5C38] to-[#124227] rounded-[20px] p-4 text-center shadow-[0_6px_20px_rgba(26,92,56,0.3)] animate-in fade-in slide-in-from-bottom-4 relative overflow-hidden">
                    <div className="absolute inset-0 opacity-20 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-white via-transparent to-transparent"></div>
                    <p className="text-[9.5px] text-emerald-200 font-bold uppercase tracking-widest relative z-10">Estimación Total</p>
                    <p className="text-[32px] font-black text-white mt-1 leading-none tracking-tight relative z-10">
                      {(Number(form.area_sown_ha) * Number(form.yield_expected)).toFixed(1)} <span className="text-xs text-emerald-200 font-bold tracking-normal">ton</span>
                    </p>
                  </div>
                )}
              </div>
            ...
            {paso === 4 && (
              <div className="space-y-6 relative z-10 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="text-center sm:text-left mb-2">
                  ...
                  <p className="text-[9.5px] text-slate-500 font-medium">Confirma los tiempos de tu cosecha.</p>
                </div>

                <div className="space-y-3.5">
                  <div className="bg-slate-50/80 rounded-[16px] p-4 border border-slate-100 shadow-sm">
                    <label className="text-[10px] font-bold text-slate-800 mb-2.5 flex items-center gap-2">
                      <Calendar size={12} className="text-slate-400"/> ¿Cuándo vas a sembrar?
                    </label>
                  ...
                  <div className="bg-slate-50/80 rounded-[16px] p-4 border border-slate-100 shadow-sm">
                    <label className="text-[10px] font-bold text-slate-800 mb-2.5 flex items-center gap-2">
                      <Clock size={12} className="text-slate-400"/> ¿Cuándo esperas cosechar? <span className="text-slate-400 font-medium ml-1 text-[9.5px]">(opcional)</span>
                    </label>
                  ...
                  <div className="pt-2">
                    <label className="block text-[10px] font-bold text-slate-800 mb-3 text-center sm:text-left">
                      ¿Destino principal de la cosecha? <span className="text-slate-400 font-medium ml-1 text-[9.5px]">(opcional)</span>
                    </label>
                        ...
                        <button key={d.valor}
                          ...
                          className={`rounded-[16px] p-3.5 border-2 text-left transition-all active:scale-95 flex flex-col gap-2.5 items-center text-center
                            ${form.destination === d.valor
                              ? 'border-[#1A5C38] bg-[#1A5C38]/5 shadow-sm'
                              : 'border-slate-100 bg-white hover:border-slate-200 hover:bg-slate-50'}`}>
                          <d.icon size={13} className={form.destination === d.valor ? 'text-[#1A5C38]' : 'text-slate-400'} strokeWidth={2} />
                          <span className={`text-[10px] font-black tracking-tight ${form.destination === d.valor ? 'text-[#1A5C38]' : 'text-slate-600'}`}>
                            {d.label}
                          </span>
                        </button>
                      ))}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-2xl border-t border-slate-200/50 pb-safe shadow-[0_-8px_20px_rgba(0,0,0,0.04)]">
        <div className="w-full max-w-[700px] mx-auto px-4 sm:px-5 py-3.5">
          <div className="max-w-[500px] mx-auto space-y-2.5">
            {paso === 1 && (
              <button onClick={() => { setError(''); setPaso(2); }}
                disabled={!form.cycle_type}
                className="w-full bg-[#1A5C38] hover:bg-[#124227] text-white py-3.5 rounded-full text-[9.5px] font-bold disabled:opacity-40 disabled:scale-100 transition-all active:scale-[0.98] shadow-[0_6px_15px_rgba(26,92,56,0.2)]">
                Continuar
              </button>
            )}
            {paso === 2 && (
            ...
            {paso === 4 && (
              <button onClick={guardar}
                ...
                className="w-full bg-[#1A5C38] hover:bg-[#124227] text-white py-3.5 rounded-full text-[9.5px] font-bold disabled:opacity-40 disabled:scale-100 transition-all active:scale-[0.98] shadow-[0_6px_15px_rgba(26,92,56,0.2)] flex items-center justify-center gap-2">
            ...
            <button onClick={saltar} className="w-full py-1.5 text-slate-400 hover:text-slate-600 text-[9.5px] font-bold transition-colors active:scale-95">
              Omitir por ahora
            </button>
          </div>
        </div>
