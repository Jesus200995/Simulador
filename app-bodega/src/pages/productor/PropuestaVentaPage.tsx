      ...
      <div className="p-4 max-w-2xl mx-auto">
        ...
        {paso === 'sin_ciclo' && (
          <div className="text-center py-16">
            <Sprout size={48} className="text-[#1e5b4f] mx-auto mb-4" />
            ...
            <button onClick={() => navigate('/productor/ciclo')}
              className="bg-[#1e5b4f] text-white px-6 py-3 rounded-2xl font-semibold">
              Registrar mi ciclo →
            </button>
          </div>
        )}
          ...
          <div className="space-y-3">
            ...
            {ups.filter(u => (u.ciclos_activos?.length || 0) > 0).map(up => (
              <button key={up.up_id} onClick={() => seleccionarUP(up)}
                className="w-full bg-white rounded-2xl border border-gray-200 p-4 text-left hover:border-[#1e5b4f] hover:bg-green-50 transition-colors flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-[#1e5b4f] mt-1">
            ...
            {ciclos.map(ciclo => (
              <button key={ciclo.cycle_id}
                ...
                <div>
                  ...
                  <p className="text-sm text-[#1e5b4f] mt-0.5 flex items-center gap-1"><Wheat size={14} /> {variedadLegible(ciclo)}</p>
          ...
          <div className="space-y-4">
            <div className="bg-green-50 border border-green-200 rounded-2xl p-4">
              ...
              <div className="space-y-1">
                ...
                <div className="flex justify-between text-sm"><span className="text-gray-600">Variedad:</span><span className="font-medium text-[#1e5b4f] inline-flex items-center gap-1"><Wheat size={14} /> {variedadLegible(cicloSeleccionado)}</span></div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <input type="number" value={volumen} onChange={e => setVolumen(e.target.value)}
                ...
                className="w-full border border-gray-200 rounded-xl px-4 py-3 text-lg font-semibold focus:outline-none focus:ring-2 focus:ring-[#1e5b4f]" />
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="relative">
                ...
                <input type="number" value={precioMinimo} onChange={e => setPrecioMinimo(e.target.value)}
                  ...
                  className="w-full border border-gray-200 rounded-xl pl-8 pr-20 py-3 text-lg font-semibold focus:outline-none focus:ring-2 focus:ring-[#1e5b4f]" />
            ...
            <button onClick={enviar} disabled={enviando || !volumen || !fechaDesde || !fechaHasta}
              className="w-full bg-[#1e5b4f] text-white py-4 rounded-2xl font-bold text-lg disabled:opacity-40 flex items-center justify-center gap-2">
