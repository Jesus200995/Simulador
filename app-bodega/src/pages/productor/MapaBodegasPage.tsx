...
export default function MapaBodegasPage() {
          ...
          <div className="space-y-2">
            <div className="bg-emerald-50 rounded-lg p-2.5 flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                <div className="w-6 h-6 rounded-md bg-[#1e5b4f]/10 flex items-center justify-center flex-shrink-0">
                  <Wheat size={12} className="text-[#1e5b4f]" />
                </div>
                <p className="text-[10px] text-gray-600 font-medium">Precio hoy</p>
              </div>
              <p className="font-black text-[#1e5b4f] text-[14px]">
          ...
          <div className="flex gap-1.5 pt-1 border-t border-gray-100">
            {b.senal_activa && (
              <button onClick={() => handleMeInteresa(b.senal_activa!.id, b.nombre)}
                className="flex-[1.5] bg-[#1e5b4f] hover:bg-[#195049] text-white text-[10px] font-black py-2 rounded-lg flex items-center justify-center gap-1 shadow-sm transition-all">
  ...
  return (
    <div className="bg-[#e8f5f3] w-full max-w-2xl mx-auto h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-shrink-0 bg-gradient-to-br from-[#1e5b4f] via-[#267a6b] to-[#2e8c7b] rounded-b-3xl shadow-[0_4px_20px_rgba(26,92,56,0.25)] z-20">
        ...
        <div className="relative h-full rounded-2xl overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.12)] ring-1 ring-black/5">
          ...
          {loadingBodegas && (
            <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10 bg-white/90 backdrop-blur-md rounded-full px-4 py-2 shadow-lg flex items-center gap-2 text-[12px] font-semibold text-gray-700">
              <div className="w-3 h-3 border-2 border-[#1e5b4f]/30 border-t-[#1e5b4f] rounded-full animate-spin" />
              Cargando bodegas...
            </div>
          )}
              ...
              <div className="text-center px-6 max-w-xs bg-white rounded-2xl p-8 shadow-2xl">
                ...
                {radioKm < 500 && (
                  <button onClick={() => setRadioKm(500)}
                    className="mt-4 px-4 py-2 bg-[#1e5b4f] text-white rounded-xl text-sm font-semibold">
                    Buscar en 500 km
                  </button>
                )}
              </div>
      ...
      {/* â”€â”€ Toast â”€â”€ */}
      {confirmacionVisible && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[3000] bg-[#1e5b4f] text-white px-5 py-3.5 rounded-2xl shadow-2xl flex items-center gap-3 max-w-xs w-full mx-4">
