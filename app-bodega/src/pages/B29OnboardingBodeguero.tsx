              ...
              >
                ...
                <div className="flex-1 min-w-0 transition-transform duration-500 group-hover/card:translate-x-1">
                  <div className="flex items-center gap-2.5">
                    <span className={`p-1.5 rounded-lg ${p.hecho ? 'bg-green-100 text-green-600' : 'bg-[#002f2a]/10 text-[#002f2a]'}`}>{p.icon}</span>
                    <p className={`text-[16px] font-bold ${p.hecho ? 'text-gray-400 line-through' : 'text-gray-900 group-hover/card:text-[#002f2a]'} transition-colors`}>
                      {i + 1}. {p.titulo}
                    </p>
                  </div>
                  <p className="text-[13px] text-gray-500 font-medium mt-1 ml-10 leading-snug">{p.desc}</p>
                </div>
                <ChevronRight size={18} className={`flex-shrink-0 transition-transform duration-300 group-hover/card:translate-x-1 ${p.hecho ? 'text-green-300' : 'text-gray-300 group-hover/card:text-[#002f2a]'}`} />
              </button>
            ))}
            ...
            {completados === total && (
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200/60 rounded-[1.5rem] p-8 text-center mt-6 shadow-[0_4px_24px_rgba(52,211,153,0.1)]">
                ...
                <button
                  ...
                  className="mt-6 px-8 py-3.5 bg-[#002f2a] text-white rounded-[1.25rem] text-[15px] font-bold active:scale-[0.98] transition-all shadow-[0_4px_12px_rgba(26,92,56,0.2)] hover:shadow-[0_8px_24px_rgba(26,92,56,0.3)] inline-flex items-center gap-2"
                >
