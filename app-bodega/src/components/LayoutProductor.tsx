...
export function LayoutProductor({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 bg-[#f4f5f7] flex flex-col w-full overflow-hidden">
      ...
      {/* ── Header ─────────────────────────────────────── */}
      <header className="flex-none z-30 bg-white/95 backdrop-blur-xl border-b border-slate-200/60 px-4 sm:px-5 h-[60px] flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="flex flex-col leading-none min-w-0">
            <span className="text-[10px] font-black tracking-tight leading-none text-[#1A5C38]">SIMAC</span>
      ...
      <div className={`fixed top-0 right-0 bottom-0 z-50 w-[300px] bg-white shadow-2xl flex flex-col transition-transform duration-300 ease-out ${drawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="bg-gradient-to-br from-[#1A5C38] to-[#2d7a52] px-5 pt-12 pb-6">
          ...
          <p className="text-white font-bold text-[10px] leading-tight">{nombres || 'Productor'}</p>
          <span className="inline-block mt-2 bg-white/20 text-white text-[9.5px] font-semibold px-2.5 py-0.5 rounded-full">
            Productor
          </span>
        </div>
...
export function PageHeaderProductor({
  ...
  const navigate = useNavigate();
  return (
    <div className="sticky top-0 z-20 w-full bg-white/95 backdrop-blur-md border-b border-slate-200/60 px-4 sm:px-5 pt-3.5 pb-4 shadow-sm">
          ...
          <div className="flex-1 min-w-0">
            {back !== undefined && (
              <button
                ...
                className="flex items-center gap-0.5 text-[#1A5C38] text-[10px] font-bold mb-2 hover:opacity-70 transition-opacity"
              >
                <ChevronLeft size={13} strokeWidth={2.5} className="-ml-1" />
                Volver
              </button>
            )}
            <h1 className="text-[10px] font-bold text-slate-900 leading-tight truncate">{title}</h1>
            {subtitle && <p className="text-[10px] text-slate-500 font-medium mt-0.5 truncate">{subtitle}</p>}
          </div>
