...
const CFG: Record<ToastType, { icon: React.ReactNode; bar: string; bg: string }> = {
  ...
  info: {
    icon: <Info size={12} className="text-blue-500 shrink-0" />,
    bar:  'bg-blue-400',
    bg:   'bg-white',
  },
};
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
        ...
        return (
          <div key={t.id}
            ...
            <div className="flex items-center gap-3 px-4 py-3.5 flex-1 min-w-0">
              {c.icon}
              <p className="flex-1 text-[10px] font-medium text-gray-800 leading-snug">{t.message}</p>
  ...
  const confirmDialog = dialog ? (
    <div
      className="animate-backdrop-in fixed inset-0 flex items-center justify-center p-4"
    ...
    >
      <div className="animate-modal-in w-full max-w-[320px] bg-white rounded-3xl shadow-[0_32px_80px_rgba(0,0,0,0.28)] overflow-hidden">
        ...
        <div className="px-5 pb-6 pt-1 text-center">
          ...
          <p className="text-[10px] font-bold text-gray-900 leading-snug">{dialog.msg}</p>
          <p className="text-[9.5px] text-gray-400 mt-2">Toca <strong>Confirmar</strong> para continuar o <strong>Cancelar</strong> para volver.</p>
        </div>
        {/* Buttons */}
        <div className="flex divide-x divide-gray-100 border-t border-gray-100">
          <button onClick={() => closeDialog(false)}
            className="flex-1 py-4 text-[10px] font-semibold text-gray-500 active:bg-gray-50 transition-colors">
            Cancelar
          </button>
          <button onClick={() => closeDialog(true)}
            className="flex-1 py-4 text-[10px] font-bold text-[#1A5C38] active:bg-green-50 transition-colors">
            Confirmar
          </button>
        </div>
      </div>
