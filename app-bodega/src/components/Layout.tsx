...
export function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex flex-col min-h-dvh bg-[#F2F2F7] overflow-x-clip overflow-y-auto w-full">
      ...
      {/* ── Header ─────────────────────────────────────── */}
      <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-xl border-b border-black/[0.06] px-4 sm:px-5 h-[60px] flex items-center justify-between shadow-sm shadow-black/[0.03]">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="flex flex-col leading-none min-w-0">
            <span className="text-[10px] font-black tracking-tight leading-none somec-shimmer">SIMAC</span>
      ...
      <div className={`fixed top-0 right-0 bottom-0 z-50 w-[300px] bg-white shadow-2xl flex flex-col transition-transform duration-300 ease-out ${drawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="bg-gradient-to-br from-[#1A5C38] to-[#2d7a52] px-5 pt-12 pb-6">
          ...
          <div className="w-14 h-14 rounded-2xl bg-white/20 flex items-center justify-center mb-3 shadow-inner">
            <span className="text-white text-[9.5px] font-bold">{initials}</span>
          </div>
          <p className="text-white font-bold text-[10px] leading-tight">{user?.nombre_completo || 'Usuario'}</p>
          <p className="text-green-200 text-[9.5px] mt-0.5">{user?.email || ''}</p>
          <span className="inline-block mt-2 bg-white/20 text-white text-[9.5px] font-semibold px-2.5 py-0.5 rounded-full capitalize">
            {user?.rol || 'bodega'}
          </span>
        </div>
        ...
        <div className="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
          {[
            ...
            <button key={label} onClick={action}
              ...
              <span className="flex-1 text-[9.5px] font-medium">{label}</span>
        ...
        <div className="px-5 py-3 border-t border-gray-100">
          <p className="text-[9.5px] text-gray-400 text-center leading-snug">
            {SYSTEM_NAME}
          </p>
        ...
        <div className="px-4 pb-8 pt-2">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-red-50 text-red-600 rounded-2xl py-3.5 text-[9.5px] font-semibold active:bg-red-100 transition-colors"
          >
            <LogOut size={13} />
            Cerrar sesión
          </button>
...
export function PageHeader({
  ...
  const navigate = useNavigate();
  return (
    <div className="w-full bg-white/90 backdrop-blur-sm border-b border-black/[0.06] px-4 sm:px-5 pt-3.5 pb-4">
          ...
          <div className="flex-1 min-w-0">
            {back !== undefined && (
              <button
                ...
                className="flex items-center gap-0.5 text-[#1A5C38] text-[10px] font-medium mb-1.5 active:opacity-60 transition-opacity"
              >
                <ChevronLeft size={13} strokeWidth={2.5} className="-ml-1" />
                Volver
              </button>
            )}
            <h1 className="text-[10px] font-bold text-gray-900 leading-tight truncate">{title}</h1>
            {subtitle && <p className="text-[9.5px] text-gray-400 mt-0.5 truncate">{subtitle}</p>}
          </div>
    ...
    <div className="w-full bg-gradient-to-br from-[#1A5C38] via-[#1e6b42] to-[#22733f] rounded-b-3xl shadow-[0_8px_30px_rgba(26,92,56,0.25)] relative overflow-hidden group/banner">
      ...
      <div className="w-full mx-auto px-4 sm:px-5 lg:px-10 xl:px-16 pt-4 pb-5 relative z-10 transition-transform duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover/banner:translate-x-1">
        {back !== undefined && (
          <button
            onClick={() => typeof back === 'number' ? navigate(back) : navigate(back)}
            className="flex items-center gap-0.5 text-green-200/80 text-[9.5px] font-medium mb-2 active:opacity-60 transition-opacity hover:text-green-100"
          >
            <ChevronLeft size={12} strokeWidth={2.5} className="-ml-1 transition-transform group-hover/banner:-translate-x-0.5" />
            Volver
          </button>
        )}
        <div className="flex items-end justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <h1 className="text-[10px] sm:text-[10px] font-bold text-white leading-tight drop-shadow-sm">{title}</h1>
            {subtitle && <p className="text-green-100/80 text-[9.5px] mt-0.5 leading-snug font-medium">{subtitle}</p>}
          </div>
