      ...
      <div class="sheet-content">
        <template v-if="mobileTab === 'filtros'">
          <div class="panel-search">
            <div class="search-input-wrap">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
              <input v-model="searchText" type="text" class="search-input" placeholder="Buscar bodega..." @input="onSearch" />
            </div>
          </div>
          ...
          <div class="panel-kpi">
            ...
            <div class="kpi-card toneladas">
              <span class="kpi-value">{{ formatNumber(kpi.total_capacidad) }}</span>
              <span class="kpi-label">Cap. Ton</span>
            </div>
            <div class="kpi-card nacional">
              <span class="kpi-value">{{ kpi.total_estados }}</span>
              <span class="kpi-label">Estados</span>
        ...
        <template v-if="mobileTab === 'lista'">
          <ul class="bodega-list">
            <li v-for="bodega in bodegas" :key="bodega.id" class="bodega-item" :class="{ active: selectedBodega?.id === bodega.id }" @click="selectBodega(bodega); sheetCollapsed = true">
              <div class="bodega-icon">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="1.5"><path d="M3 21V8l9-5 9 5v13"/><path d="M9 21V13h6v8"/></svg>
              </div>
              <div class="bodega-info">
...
function addMarkers() {
  ...
  bodegas.value.forEach((bodega) => {
    ...
    `
    el.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M3 21V8l9-5 9 5v13"/><path d="M9 21V13h6v8"/></svg>'
    el.addEventListener('mouseenter', () => { el.style.transform = 'scale(1.25)'; el.style.boxShadow = '0 4px 14px rgba(105,28,50,0.45)' })
    ...
    const popupHtml = `
      ...
      <div class="popup-body">
        <div class="popup-row"><span>DDR</span><strong>${sanitize(bodega.ddr || '-')}</strong></div>
        ...
        <div class="popup-row"><span>Capacidad</span><strong>${formatNumber(bodega.capacidad_toneladas)} ton</strong></div>
        <button class="popup-btn" onclick="window.__verDetalle(${bodega.id})">Ver detalle →</button>
      </div>
    `

    const popup = new mapboxgl.Popup({ offset: 18, closeButton: true, maxWidth: '260px' }).setHTML(popupHtml)
    ...
    marker.getElement().addEventListener('click', () => { selectedBodega.value = bodega })
    markers.push(marker)
  })
}
...
function selectBodega(bodega: Bodega) {
  ...
  if (!map) return
  map.flyTo({ center: [bodega.longitud, bodega.latitud], zoom: 12, duration: 1200, essential: true })
